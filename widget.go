...
func (w *CWidget) Init() (already bool) {
	...
	w.fcHandle = fmt.Sprintf("%v.focus-changed", w.ObjectName())
	w.Connect(SignalLostFocus, w.fcHandle, w.handleLostFocus)
	w.Connect(SignalGainedFocus, w.fcHandle, w.handleGainedFocus)
	return false
}
...
func (w *CWidget) Unparent() {
	if w.parent != nil {
		if parent, ok := w.parent.(Container); ok {
			if f := w.Emit(SignalUnparent, w, w.parent); f == cdk.EVENT_PASS {
				parent.Remove(w)
			}
		}
	}
...
func (w *CWidget) Show() {
	if !w.HasFlags(VISIBLE) {
		if r := w.Emit(SignalShow, w); r == cdk.EVENT_PASS {
			w.SetFlags(VISIBLE)
			w.Invalidate()
		}
	}
...
func (w *CWidget) Hide() {
	if w.HasFlags(VISIBLE) {
		if r := w.Emit(SignalHide, w); r == cdk.EVENT_PASS {
...
func (w *CWidget) ShowAll() {
	if !w.HasFlags(VISIBLE) {
		if r := w.Emit(SignalShowAll, w); r == cdk.EVENT_PASS {
			w.SetFlags(VISIBLE)
			w.Invalidate()
		}
	}
...
// AccelGroupConnectByPath or AccelGroupConnect. The
// closures can be used to monitor accelerator changes on widget , by
// connecting to the AccelGroup ::accel-changed signal of the
...
func (w *CWidget) Reparent(parent Container) {
	if r := w.Emit(SignalReparent, w, parent); r == cdk.EVENT_PASS {
		if w.parent != nil {
...
func (w *CWidget) GrabFocus() {
	if w.CanFocus() {
		if r := w.Emit(SignalGrabFocus, w); r == cdk.EVENT_PASS {
			tl := w.GetWindow()
			if tl != nil {
				var fw Widget
				focused := tl.GetFocus()
				...
				if focused != nil {
					...
					if fw, ok = focused.(Widget); ok && fw.ObjectID() != w.ObjectID() {
						if f := fw.Emit(SignalLostFocus, focused); f == cdk.EVENT_STOP {
							fw = nil
						}
					}
				}
				if f := w.Emit(SignalGainedFocus, w, focused); f == cdk.EVENT_STOP {
					if fw != nil {
						tl.SetFocus(focused)
					}
				}
...
func (w *CWidget) SetState(state StateType) {
	if f := w.Emit(SignalSetState, w, state); f == cdk.EVENT_PASS {
...
func (w *CWidget) SetSensitive(sensitive bool) {
	if f := w.Emit(SignalSetSensitive, w, sensitive); f == cdk.EVENT_PASS {
		if !sensitive {
...
func (w *CWidget) SetParent(parent Container) {
	if f := w.Emit(SignalSetParent, w, w.parent, parent); f == cdk.EVENT_PASS {
		if w.HasFlags(PARENT_SENSITIVE) && w.parent != nil {
			if parent, ok := w.parent.(Widget); ok {
				_ = parent.Disconnect(SignalLostFocus, w.fcHandle)
				_ = parent.Disconnect(SignalGainedFocus, w.fcHandle)
			}
		}
		if err := w.SetStructProperty(PropertyParent, parent); err != nil {
			...
			if cw, ok := parent.(Widget); parent != nil && ok && w.HasFlags(PARENT_SENSITIVE) {
				cw.Connect(SignalLostFocus, w.fcHandle, w.handleLostFocus)
				cw.Connect(SignalGainedFocus, w.fcHandle, w.handleGainedFocus)
			}
		}
	}
}
...
// func (w *CWidget) SetExtensionEvents(mode ExtensionMode) {
// 	if err := w.SetStructProperty(PropertyExtensionEvents, mode); err != nil {
// 		w.LogErr(err)
// 	}
// }
...
// func (w *CWidget) GetExtensionEvents() (value ExtensionMode) {
// 	var err error
// 	if value, err = w.GetStructProperty(PropertyExtensionEvents); err != nil {
// 		w.LogErr(err)
// 	}
// 	return
// }
...
// stack will be used to create all widgets. Remove cmap with
// PopColormap. There's little reason to use this function.
// Parameters:
// 	cmap	a Colormap
...
// PushColormap is a better function to use if you only want to
...
func (w *CWidget) SizeRequest() cdk.Rectangle {
	return cdk.MakeRectangle(w.GetSizeRequest())
}
...
func (w *CWidget) SetSizeRequest(width, height int) {
	if f := w.Emit(SignalSetSizeRequest, w, cdk.MakeRectangle(width, height)); f == cdk.EVENT_PASS {
...
// LabelSetMnemonicWidget). The widgets in the list are not
...
// Create a Pixmap of the contents of the widget and its children. Works
// even if the widget is obscured. The depth and visual of the resulting
...
// PixbufGetFromDrawable can be used to convert the pixmap to a
// visual independant representation. The snapshot area used by this function
// is the widget 's allocation plus any extra space occupied by additional
// windows belonging to this widget (such as the arrows of a spin button).
// Thus, the resulting snapshot pixmap is possibly larger than the
// allocation. If clip_rect is non-NULL, the resulting pixmap is shrunken to
...
// negative, the width or height of the resulting pixmap will be shrunken by
// the respective amount. For instance a clip_rect { +5, +5, -10, -10 } will
// chop off 5 pixels at each side of the snapshot pixmap. If non-NULL,
// clip_rect will contain the exact widget-relative snapshot coordinates upon
// return. A clip_rect of { -1, -1, 0, 0 } can be used to preserve the
// auto-grown snapshot area and use clip_rect as a pure output parameter. The
...
// func (w *CWidget) GetAllocation(allocation cdk.Rectangle) {}
...
// func (w *CWidget) SetAllocation(allocation cdk.Rectangle) {}
...
func (w *CWidget) SetWindow(window Window) {
	if f := w.Emit(SignalSetWindow, w, window); f == cdk.EVENT_PASS {
...
func (w *CWidget) SetTheme(theme cdk.Theme) {
	if theme.String() != w.GetTheme().String() {
		if f := w.Emit(SignalSetTheme, w, theme); f == cdk.EVENT_PASS {
...
func (w *CWidget) UnsetState(v StateType) {
	if f := w.Emit(SignalUnsetState, w, v); f == cdk.EVENT_PASS {
		w.state = w.state &^ v
	}
}
...
func (w *CWidget) UnsetFlags(v WidgetFlags) {
	if f := w.Emit(SignalUnsetFlags, w, v); f == cdk.EVENT_PASS {
		w.flags = w.flags &^ v
	}
}
...
func (w *CWidget) SetFlags(v WidgetFlags) {
	if f := w.Emit(SignalSetFlags, w, w.flags, v); f == cdk.EVENT_PASS {
		w.flags = w.flags | v
	}
}
...
func (w *CWidget) GrabEventFocus() {
	if window := w.GetWindow(); window != nil {
		if f := w.Emit(SignalGrabEventFocus, w, window); f == cdk.EVENT_PASS {
	...
	if window := w.GetWindow(); window != nil {
		if ef := window.GetEventFocus(); ef != nil {
			if wef, ok := ef.(Widget); ok && wef.ObjectID() == w.ObjectID() {
				if f := w.Emit(SignalReleaseEventFocus, w, window); f == cdk.EVENT_PASS {
...
func (w *CWidget) handleLostFocus(_ []interface{}, _ ...interface{}) cdk.EventFlag {
	theme := w.GetTheme()
...
func (w *CWidget) handleGainedFocus(_ []interface{}, _ ...interface{}) cdk.EventFlag {
	theme := w.GetTheme()
	theme.Content.Normal = theme.Content.Focused
	theme.Border.Normal = theme.Border.Focused
	w.SetThemeRequest(theme)
