...
type CContainer struct {
	...
	children      []Widget
	resizeMode    cdk.ResizeMode
	fcHandle      string
	properties    []*cdk.CProperty
	property      map[int][]*cdk.CProperty
...
func (c *CContainer) Init() (already bool) {
	...
	c.SetFlags(PARENT_SENSITIVE)
	c.SetFlags(APP_PAINTABLE)
	...
	c.property = make(map[int][]*cdk.CProperty)
	c.focusChain = make([]interface{}, 0)
	c.fcHandle = fmt.Sprintf("%v.focus-changed", c.ObjectName())
	c.Connect(SignalLostFocus, c.fcHandle, c.handleLostFocus)
	c.Connect(SignalGainedFocus, c.fcHandle, c.handleGainedFocus)
	return false
}
...
func (c *CContainer) Add(w Widget) {
	...
	if f := c.Emit(SignalAdd, c, w); f == cdk.EVENT_PASS {
		cdk.DebugDF(1, "Container.Add(%v)", w)
		w.SetParent(c)
		...
		w.Connect(SignalLostFocus, c.fcHandle, c.handleLostFocus)
		w.Connect(SignalGainedFocus, c.fcHandle, c.handleGainedFocus)
...
func (c *CContainer) Remove(w Widget) {
	...
	for _, child := range c.children {
		if child.ObjectID() == w.ObjectID() {
			if f := c.Emit(SignalRemove, c, child); f == cdk.EVENT_PASS {
				_ = w.Disconnect(SignalLostFocus, c.fcHandle)
				_ = w.Disconnect(SignalGainedFocus, c.fcHandle)
				w.SetParent(nil)
...
func (c *CContainer) SetOrigin(x, y int) {
	if f := c.Emit(SignalOrigin, c, cdk.MakePoint2I(x, y)); f == cdk.EVENT_PASS {
		c.origin.Set(x, y)
...
func (c *CContainer) handleLostFocus(data []interface{}, argv ...interface{}) cdk.EventFlag {
	c.Invalidate()
	return cdk.EVENT_PASS
}

func (c *CContainer) handleGainedFocus(data []interface{}, argv ...interface{}) cdk.EventFlag {
	c.Invalidate()
	return cdk.EVENT_PASS
}
