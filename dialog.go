...
func (d *CDialog) Init() (already bool) {
	...
	d.SetFlags(PARENT_SENSITIVE)
	d.SetFlags(APP_PAINTABLE)
	d.parent = d
	d.defResponse = ResponseNone
	d.GetVBox().Show()
	d.content = NewVBox(false, 0)
	d.content.Show()
	d.GetVBox().PackStart(d.content, true, true, 0)
	d.action = NewHButtonBox(false, 0)
	// d.action.SetSizeRequest(-1, 3)
	d.action.Show()
	d.GetVBox().PackEnd(d.action, false, true, 0)
	d.handle = fmt.Sprintf("%v.response", d.ObjectName())
	d.done = make(chan bool, 1)
	d.response = ResponseNone
	d.Connect(SignalResponse, d.handle, func(data []interface{}, argv ...interface{}) cdk.EventFlag {
		if len(argv) == 1 {
...
func (d *CDialog) Run() (response chan ResponseType) {
	...
	parentId := -1
	dw, dh := display.Size()
	previousWindow := dm.ActiveWindow()
	if transient := d.GetTransientFor(); transient != nil {
		isOverlay = true
		dm.AddWindowOverlay(transient.ObjectID(), d, d.getDialogRegion())
		d.Resize()
		dm.SetActiveWindow(transient)
		parentId = transient.ObjectID()
	} else {
		d.SetAllocation(cdk.MakeRectangle(dw, dh))
		d.Resize()
		dm.SetActiveWindow(d)
	}
	dm.RequestDraw()
	dm.RequestSync()
	go func() {
		...
		response <- d.response
		if isOverlay && parentId > -1 {
			dm.RemoveWindowOverlay(parentId, d.ObjectID())
		}
		dm.SetActiveWindow(previousWindow)
		dm.RequestDraw()
		dm.RequestSync()
	}()
	return
...
func (d *CDialog) AddButton(buttonText string, responseId ResponseType) (value Button) {
	if item := LookupStockItem(StockID(buttonText)); item != nil {
		value = NewButtonFromStock(StockID(buttonText))
	} else {
		value = NewButtonWithLabel(buttonText)
	}
	value.Show()
	d.AddActionWidget(value, responseId)
	return
}
...
func (d *CDialog) AddActionWidget(child Widget, responseId ResponseType) {
	handle := fmt.Sprintf("%v.activate", d.ObjectName())
	child.Connect(SignalActivate, handle, func(data []interface{}, argv ...interface{}) cdk.EventFlag {
...
func (d *CDialog) ProcessEvent(evt cdk.Event) cdk.EventFlag {
	...
	case *cdk.EventKey:
		switch e.Key() {
		case cdk.KeyEscape:
			if f := d.Emit(SignalClose); f == cdk.EVENT_PASS {
	...
	case *cdk.EventResize:
		...
		return d.Resize()
	case *cdk.EventMouse:
		if f := d.Emit(SignalEventMouse, d, evt); f == cdk.EVENT_PASS {
			if child := d.GetChild(); child != nil {
				if mw := child.GetWidgetAt(cdk.NewPoint2I(e.Position())); mw != nil {
					if ms, ok := mw.(Sensitive); ok && ms.IsSensitive() && ms.IsVisible() {
						if f := ms.ProcessEvent(evt); f == cdk.EVENT_STOP {
							return cdk.EVENT_STOP
						}
					}
				}
...
func (d *CDialog) getDialogRegion() (region cdk.Region) {
	if dm := cdk.GetDisplayManager(); dm != nil {
		var origin cdk.Point2I
		alloc := cdk.MakeRectangle(dm.Display().Size())
		if tf := d.GetTransientFor(); tf != nil {
			req := d.SizeRequest()
			...
			region = cdk.MakeRegion(origin.X, origin.Y, req.W, req.H)
		} else {
			region = cdk.MakeRegion(origin.X, origin.Y, alloc.W, alloc.H)
		}
	}
	return
}
...
func (d *CDialog) Resize() cdk.EventFlag {
	...
	if tf := d.GetTransientFor(); tf != nil {
		if dm := d.GetDisplayManager(); dm != nil {
			dm.SetWindowOverlayRegion(tf.ObjectID(), d.ObjectID(), *region)
		}
	}
	if f := d.Emit(SignalResize, d, region.W, region.H); f == cdk.EVENT_PASS {
		if child := d.GetChild(); child != nil {
			origin := cdk.MakePoint2I(1, 1)
...
func (d *CDialog) Invalidate() cdk.EventFlag {
	// d.rebuildFocusChain()
	d.canvas.SetOrigin(cdk.MakePoint2I(1, 1))
	alloc := d.GetAllocation()
	alloc.Sub(2, 2)
	d.canvas.Resize(alloc, d.GetThemeRequest().Content.Normal)
...
func (d *CDialog) Draw(canvas cdk.Canvas) cdk.EventFlag {
	...
	if !d.IsVisible() || size.W == 0 || size.H == 0 {
		d.LogDebug("Dialog.Draw(): not visible, zero width or zero height")
		return cdk.EVENT_PASS
	}
	d.LogTrace("Dialog.Draw(): %v", size)
	if d.GetTitle() != "" {
		canvas.FillBorderTitle(false, d.GetTitle(), cdk.JUSTIFY_CENTER, d.GetThemeRequest())
	} else {
		canvas.FillBorder(false, true, d.GetThemeRequest())
	}
	child := d.GetChild()
	if child != nil && child.IsVisible() {
		// child.SetBoolProperty(cdk.PropertyDebug, true)
		child.Draw(d.canvas)
		if err := canvas.Composite(d.canvas); err != nil {
			d.LogError("composite error: %v", err)
		}
	}
	if debug, _ := d.GetBoolProperty(cdk.PropertyDebug); debug {
		canvas.DebugBox(cdk.ColorNavy, d.ObjectInfo())
	}
	return cdk.EVENT_STOP
}
