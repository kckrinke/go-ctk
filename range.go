...
func (r *CRange) Init() (already bool) {
	...
	r.SetFlags(SENSITIVE | PARENT_SENSITIVE)
...
func (r *CRange) SetFillLevel(fillLevel float64) {
	if err := r.SetFloatProperty(PropertyFillLevel, utils.ClampF(fillLevel, 0.0, 1.0)); err != nil {
		r.LogErr(err)
	}
}
...
func (r *CRange) SetValue(value int) {
	if adjustment := r.GetAdjustment(); adjustment != nil {
		if r.GetRestrictToFillLevel() {
			...
			value = utils.ClampI(value, 0, max)
		}
		value = utils.ClampI(value, adjustment.GetLower(), adjustment.GetUpper())
