...
func glade(ctx *cli.Context) error {
	...
	if !utils.IsFile(gladeFile) {
		fmt.Printf("file not found: %v\n", gladeFile)
	} else if gladeFile[len(gladeFile)-6:] != ".glade" {
		fmt.Printf("not a .glade interface file: %v\n", gladeFile)
	}
	app := cdk.NewApp("ctk-glade", "", "", "ctk-glade", "CTK Glade", "/dev/tty", func(d cdk.DisplayManager) error {
...
func ProcessFile(ctx *cli.Context, path string, dm cdk.DisplayManager) (err error) {
	var bytes []byte
	...
	for _, bi := range builder.GetWidgetsBuiltByType(ctk.TypeButton) {
		if button, ok := bi.(ctk.Button); ok {
			button.Connect(ctk.SignalActivate, fmt.Sprintf("glade.activate"), func(data []interface{}, argv ...interface{}) cdk.EventFlag {
...
func setupUi(builder ctk.Builder, widget interface{}, dm cdk.DisplayManager) error {
	if widget != nil {
...
func setupUiDialog(dialog ctk.Dialog, dm cdk.DisplayManager) error {
