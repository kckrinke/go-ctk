...
type CWindow struct {
	...
	displayManager cdk.DisplayManager
	canvas         cdk.Canvas
...
func (w *CWindow) Init() (already bool) {
	...
	w.SetFlags(TOPLEVEL | SENSITIVE)
	w.prevMouseEvent = cdk.NewEventMouse(0, 0, 0, 0)
	w.parent = nil
	w.displayManager = cdk.GetDisplayManager()
	w.origin.X = 0
	w.origin.Y = 0
	w.SetTheme(cdk.DefaultColorTheme)
	w.SetParent(w)
	w.SetWindow(w)
	...
	w.canvas = cdk.NewCanvas(cdk.Point2I{}, cdk.Rectangle{}, cdk.DefaultColorTheme.Content.Normal)
	_ = w.GetVBox()
	w.hoverFocus = nil
	w.Invalidate()
	return false
...
func (w *CWindow) IsActive() (active bool) {
	if aw := w.displayManager.ActiveWindow(); aw != nil {
		if aw.ObjectID() == w.ObjectID() {
			return true
		}
		if parent := w.GetParent(); parent != nil {
			if pw, ok := parent.(Window); ok {
				if overlays := w.displayManager.GetWindowOverlays(pw.ObjectID()); overlays != nil {
...
func (w *CWindow) GetSize() (width, height int) {
	return w.displayManager.Display().Size()
}
...
func (w *CWindow) Resize() cdk.EventFlag {
	...
	if f := w.Emit(SignalResize, w, size.W, size.H); f == cdk.EVENT_PASS {
		if child := w.GetChild(); child != nil {
			...
			w.canvas.SetOrigin(childOrigin)
			w.canvas.Resize(size, w.GetTheme().Content.Normal)
			child.Resize()
			w.Invalidate()
		}
	}
...
func (w *CWindow) GetDisplayManager() (dm cdk.DisplayManager) {
	if w.displayManager == nil {
		w.displayManager = cdk.GetDisplayManager()
	}
	return w.displayManager
}
...
func (w *CWindow) GetEventFocus() (o interface{}) {
	if dm := w.GetDisplayManager(); dm != nil {
		o = dm.GetEventFocus()
	}
	return
}
...
func (w *CWindow) SetEventFocus(o interface{}) {
	if f := w.Emit(SignalSetEventFocus, w, o); f == cdk.EVENT_PASS {
		if dm := w.GetDisplayManager(); dm != nil {
...
func (w *CWindow) ProcessEvent(evt cdk.Event) cdk.EventFlag {
	...
	switch e := evt.(type) {
	case *cdk.EventError:
		if f := w.Emit(SignalError, w, e); f == cdk.EVENT_PASS {
			w.LogError(e.Error())
		}
	case *cdk.EventKey:
		if f := w.Emit(SignalEventKey, w, e); f == cdk.EVENT_PASS {
			...
			if fi := w.GetFocus(); fi != nil {
				if sw, ok := fi.(Sensitive); ok && sw.IsSensitive() && sw.IsVisible() {
					...
					if f := sw.ProcessEvent(evt); f == cdk.EVENT_STOP {
						return cdk.EVENT_STOP
					}
				}
			}
	...
	case *cdk.EventMouse:
		...
		if f := w.Emit(SignalEventMouse, w, e); f == cdk.EVENT_PASS {
			if mw := w.GetWidgetAt(cdk.NewPoint2I(e.Position())); mw != nil {
				if w.hoverFocus != nil {
	...
	case *cdk.EventResize:
		alloc := cdk.MakeRectangle(w.GetDisplayManager().Display().Size())
		origin := cdk.MakePoint2I(0, 0)
		w.SetAllocation(alloc)
		w.SetOrigin(origin.X, origin.Y)
		w.canvas.SetOrigin(origin)
		w.canvas.Resize(alloc, w.GetThemeRequest().Content.Normal)
		if f := w.Emit(SignalResize, w, e, origin, alloc); f == cdk.EVENT_PASS {
...
func (w *CWindow) Invalidate() cdk.EventFlag {
	...
	if child := w.GetChild(); child != nil {
		...
		childOrigin.SubPoint(w.GetOrigin())
		w.canvas.SetOrigin(childOrigin)
		size := child.GetAllocation()
		w.canvas.Resize(size, w.GetThemeRequest().Content.Normal)
	} else {
		w.canvas.Resize(alloc, w.GetThemeRequest().Content.Normal)
	}
...
func (w *CWindow) Draw(canvas cdk.Canvas) cdk.EventFlag {
	...
	if !w.IsVisible() || size.W == 0 || size.H == 0 {
		w.LogDebug("Window.Draw(): not visible, zero width or zero height")
		return cdk.EVENT_PASS
	}
	w.LogTrace("Window.Draw(): %v", w.GetAllocation())
	if w.GetTitle() != "" {
		canvas.FillBorderTitle(false, w.GetTitle(), cdk.JUSTIFY_CENTER, w.GetThemeRequest())
	} else {
		canvas.FillBorder(false, true, w.GetThemeRequest())
	}
	child := w.GetChild()
	if child != nil && child.IsVisible() {
		child.Draw(w.canvas)
		if err := canvas.Composite(w.canvas); err != nil {
			w.LogError("composite error: %v", err)
		}
	}
	if debug, _ := w.GetBoolProperty(cdk.PropertyDebug); debug {
		canvas.DebugBox(cdk.ColorNavy, w.ObjectInfo())
	}
	return cdk.EVENT_STOP
}
